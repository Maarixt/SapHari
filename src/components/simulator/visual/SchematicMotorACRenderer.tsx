/**
 * Schematic AC Motor: circle with "M", sine wave indicator.
 * Pins L (a) and N (b). DC placeholder: state when selected shows I, P, direction.
 */
...
function sinePoints(): number[] {
  const pts: number[] = [];
  const steps = 12;
  for (let i = 0; i <= steps; i++) {
    const t = i / steps;
    const x = SINE_LEFT + t * (SINE_RIGHT - SINE_LEFT);
    const y = SINE_Y + SINE_AMP * Math.sin(t * Math.PI * 2);
    pts.push(x, y);
  }
  return pts;
}

const SINE = sinePoints();

export interface SchematicMotorACRendererProps {
  comp: SimComponent;
...
export function SchematicMotorACRenderer({
  comp,
  isSelected,
  onSelect,
  onDragEnd,
  ...
  const content = (
    <>
      ...
      <Line points={[SINE_LEFT, SINE_Y, SINE_LEFT + 2, SINE_Y]} stroke={stroke} strokeWidth={SW} strokeScaleEnabled={false} listening={false} />
      <Line points={SINE} stroke={stroke} strokeWidth={SW} strokeScaleEnabled={false} listening={false} />
      <Line points={[SINE_RIGHT - 2, SINE_Y, SINE_RIGHT, SINE_Y]} stroke={stroke} strokeWidth={SW} strokeScaleEnabled={false} listening={false} />
      ...
      <Text x={PA.x - 6} y={PA.y - 18} text="L" fontSize={8} fill={stroke} listening={false} />
      ...
      <Text x={PB.x - 6} y={PB.y - 18} text="N" fontSize={8} fill={stroke} listening={false} />
    </>
  );
