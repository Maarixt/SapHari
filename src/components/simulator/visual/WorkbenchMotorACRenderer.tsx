/**
 * Workbench AC Motor: same footprint as DC (80x70), pins on centerline.
 * Pins are geometry-based: normalized (nx, ny), rendered as Group children.
 * DC placeholder: spins when current flows. Hit rect + listening for drag/select.
 */
...
const MOTOR_PINS: NormalizedPin[] = [
  { id: 'P', nx: 0, ny: 0.5, label: 'L' },
  { id: 'N', nx: 1, ny: 0.5, label: 'N' },
];
...
export interface WorkbenchMotorACRendererProps {
  comp: SimComponent;
  isSelected: boolean;
  ...
  /** When true, draw wire-mode snap circles on pins so they align with the pads. */
  isWiringMode?: boolean;
}

export function WorkbenchMotorACRenderer({
  comp,
  isSelected,
  onSelect,
  onDragEnd,
  ...
  return (
    ...
    >
      ...
      <Group ref={rotorRef} x={ANCHOR_X} y={ANCHOR_Y} listening={false}>
        ...
        <Line points={[-ROTOR_R, 0, ROTOR_R, 0]} stroke={AC_COLOR} strokeWidth={2.5} opacity={1} />
        <Line points={[0, -ROTOR_R, 0, ROTOR_R]} stroke={AC_COLOR} strokeWidth={2.5} opacity={1} />
      </Group>
      <Circle
      ...
      <Text x={ANCHOR_X - 12} y={ANCHOR_Y + HOUSING_R - 14} text="AC" fontSize={9} fill={AC_COLOR} listening={false} opacity={1} />
      ...
      {MOTOR_PINS.map((p) => {
        ...
        return (
          <React.Fragment key={p.id}>
            ...
            <Text x={pos.x - 8} y={pos.y - 20} text={p.label} fontSize={9} fill="#94a3b8" listening={false} opacity={1} />
          </React.Fragment>
        );
      })}
    </Group>
